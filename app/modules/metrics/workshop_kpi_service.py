...
from sqlalchemy import select, func, and_, text, case, Numeric
...
class WorkshopKPIService:
    ...
    def _dates(self, start_date: Optional[datetime], end_date: Optional[datetime]):
        ...
        # Set end_date to end of day to include all incidents from that day
        end_date = end_date.replace(hour=23, minute=59, second=59, microsecond=999999) if end_date else end_date
    ...
    async def get_avg_assignment_time(
        ...
        assigned_statuses = ["asignado", "aceptado", "en_camino", "en_proceso", "completado", "resuelto"]
        total = await self.session.scalar(
            select(func.count(Incidente.id)).where(
                and_(
                    Incidente.taller_id == workshop_id,
                    Incidente.estado_actual.in_(assigned_statuses),
    ...
    async def get_avg_arrival_time(
        ...
        start_date, end_date = self._dates(start_date, end_date)
        arrived_statuses = ["en_camino", "en_proceso", "completado", "resuelto"]
        total = await self.session.scalar(
            select(func.count(Incidente.id)).where(
                and_(
                    Incidente.taller_id == workshop_id,
                    Incidente.estado_actual.in_(arrived_statuses),
                    Incidente.created_at >= start_date,
                    Incidente.created_at <= end_date,
                )
            )
    ...
    async def get_efficiency_ranking(
        ...
        result = await self.session.execute(
            select(
                ...
                func.sum(
                    case((Incidente.estado_actual == 'resuelto', 1), else_=0)
                ).label('resueltos'),
                func.round(
                    func.avg(
                        case(
    ...
    async def get_hotspots(
        ...
        result = await self.session.execute(
            select(
                func.round(Incidente.latitude.cast(Numeric), 3).label('lat'),
                func.round(Incidente.longitude.cast(Numeric), 3).label('lng'),
                func.count(Incidente.id).label('total'),
                func.string_agg(
    ...
    async def get_sla_compliance(
        ...
        tiempo_esperado_result = await self.session.scalar(
            select(func.round(func.avg(ServicioTaller.tiempo_estimado_min).cast(Numeric), 2))
            .where(
                and_(
        ...
        # Count ALL resolved incidents (by status), not just those with timestamps
        total_resueltos = await self.session.scalar(
            select(func.count(Incidente.id)).where(
                and_(
                    Incidente.taller_id == workshop_id,
                    Incidente.estado_actual.in_(["resuelto", "completado"]),
                    Incidente.created_at >= start_date,
                    Incidente.created_at <= end_date,
                )
            )
        ...
        # Calculate SLA only for incidents that have both timestamps
        result = await self.session.execute(
            select(
                func.count(Incidente.id).label('total_con_timestamps'),
                func.sum(
                    case((
                ...
                func.round(
                    func.avg(self._minutes_between(Incidente.resolved_at, Incidente.assigned_at)).cast(Numeric), 2
        ...
        resolucion_stats = result.one()

        total_con_timestamps = int(resolucion_stats.total_con_timestamps or 0)
        ...
        return {
            "total_evaluables": total_resueltos,
            "dentro_de_sla": dentro_sla,
            "fuera_de_sla": total_resueltos - dentro_sla,
            "cumplimiento_sla_pct": round(dentro_sla / total_resueltos * 100, 1) if total_resueltos > 0 else 0.0,
    ...
    async def get_dashboard(
        ...
        try:
            ...
            logger.info(
                "KPI Dashboard for workshop",
                workshop_id=workshop_id,
                asignacion=kpi_asignacion,
                llegada=kpi_llegada,
                tipos=len(kpi_tipos),
                cancelados=kpi_cancelados,
                sla_pct=kpi_sla.get("cumplimiento_sla_pct", 0),
                zonas=len(kpi_zonas),
            )
            return {
                "workshop_id": workshop_id,
                ...
                "kpi_asignacion": kpi_asignacion,
                "kpi_llegada": kpi_llegada,
                "kpi_tipos": kpi_tipos,
            ...
            logger.error("Error building KPI dashboard", exc_info=True)
            raise
